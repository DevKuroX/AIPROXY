...
func (s *PGStore) SaveProxy(p *Proxy) error {
...
func (s *PGStore) GetProxies(filter map[string]interface{}) ([]*Proxy, error) {
	ctx := context.Background()
...
func (s *PGStore) DeleteProxy(id string) error {
	_, err := s.pool.Exec(context.Background(), "DELETE FROM proxies WHERE id = $1", id)
	return err
}

func (s *PGStore) SavePool(p *ProxyPool) error {
	ctx := context.Background()
	_, err := s.pool.Exec(ctx, `
...
func (s *PGStore) GetPools() ([]*ProxyPool, error) {
	ctx := context.Background()
...
func (s *PGStore) DeletePool(id string) error {
	_, err := s.pool.Exec(context.Background(), "DELETE FROM proxy_pools WHERE id = $1", id)
	return err
}

func (s *PGStore) SaveSettings(ps *ProxySettings) error {
	ctx := context.Background()
...
func (s *PGStore) GetSettings() (*ProxySettings, error) {
	ctx := context.Background()
	ps := &ProxySettings{}
